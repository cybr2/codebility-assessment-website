    ...
    return (
        <MainLayout>
            <div className="max-w-4xl mx-auto p-8">
                <h2 className="text-4xl font-bold mb-6">{privacyPolicy.title}</h2>
                <p className="text-gray-600 text-sm  mb-12">Last Updated: {privacyPolicy.updated}</p>
                <p className="text-gray-700 mb-6" dangerouslySetInnerHTML={{ __html: privacyPolicy.notice }} />

                {privacyPolicy.sections.map((section: ITermsSection, index: number) => (
                ...
                <p className="text-gray-700 mt-12">{privacyPolicy.description}</p>
            </div>
        </MainLayout>
    );
};
